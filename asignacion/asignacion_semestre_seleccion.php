...
{
    if (($carrera == 1 || $carrera == 3) && $carnet > 202500000) {
        return true;
    }
    return false;
}
    ...
    if (DB::isError($db)) {
        ...
        if ($db->isError($inscripcion)) {
            ...
            foreach ($inscripcion as $in) {
               ...
               /* $carrera = $in[carrera];
                if ($extension == 0 && in_array($carrera, [3]) && !getJornadaSeleccionada($db, $extension, $anio, $semestre, $evaluacion, $carnet)) {
                            ...
                            if (count($repitencia) <> 0) {
                                ...
                                if ($ciclo == 0) {
                                    ...
                                    if ($ciclo[fecha_inicio_asignacion] > $ciclo[fecha_actual] && !in_array($carnet, $habilitarCarnet)) {
                                        ...
                                        $mensaje = "El período de asignación de cursos no se encuentra habilitado, por favor verifique en el calendario oficiall. E02.";
...
GROUP BY secx.`codigo`) as secciones_m
        ...
        if (!$error && !$aviso) {
            ...
            if (!empty($a)) {
                ...
                $determinar_jornada = getJornadaSeleccionada($db, $extension, $anio, $semestre, $evaluacion, $carnet);
                //$determinar_jornada = false; //NO VERIFICAR JORNADA
                //var_dump($a); die;
                foreach ($a as $cu) {
                    ...
                    if (!empty($seccion)) {
                       ...
                       /* $seccionesCupo = "A,A1,A2,B,C,C1,C2,D,F,F1,F2,G,G1,H,M1,PGRAD,V1";*/
