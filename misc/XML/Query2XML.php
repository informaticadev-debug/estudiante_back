 ...
 * @version $Id: Query2XML.php,v 1.36 2006/04/22 23:54:57 lukasfeiler Exp $
 */
...
class XML_Query2XML {
    ...
    public function stopProfiling() {
        ...
        if (isset($this->_profile['start']) && $this->_profile['stop'] == 0) {
            ...
            $this->_profile['duration'] = $this->_profile['stop'] - $this->_profile['start'];
        }
    }
        ...
        if (!isset($tree[$id])) {
            ...
            foreach ($attributes as $attributeName => $column) {
                if (is_array($column)) {
                    ...
                    'The attribute "'
                    . $attributeName
                    . '" was not specified using a string nor an array', 'attributes'
                    );
                }
            }
            if (isset($options['value'])) {
            ...
            foreach ($elements as $tagName => $column) {
                if (is_array($column)) {
                    ...
                    $this->_processComplexElementSpecification(
                            $record, $elements[$tagName], $dom, $tree[$id], $tagName, $idColumn, $mapper
                    );
                } else {
                    //simple element specification
                    $tagValue = $this->_applyColumnStringToRecord(
                            $column, $record, 'elements'
                    );
    ...
    private function _processComplexElementSpecification(&$record, &$options, $dom, &$tree, $tagName, $parentIdColumn, $parentMapper) {
        $tag = $tree['tag'];
        ...
        try {
            ...
            for ($i = 0; $i < count($records); $i++) {
                self::_addDOMGrandchildren(
                        $tag, $this->_getNestedXMLRecord(
                                $records[$i], $options, $dom, $tree['elements'][$tagName]
                        ), $options['rootTag']
                );
            }
        } catch (XML_Query2XML_ConfigException $e) {
    ...
    private function _processComplexAttributeSpecification($attributeName, &$record, &$options, $tag) {
        if (isset($options['condition'])) {
            if (!eval('return ' . $options['condition'] . ';')) {
        ...
        if (!isset($options['value'])) {
            ...
            . 'specification', array('attributes', $attributeName)
            );
        }
        ...
        $attributeValue = $this->_applyColumnStringToRecord(
                $options['value'], $attributeRecord, 'attributes'
        );
    ...
    private function _applySqlOptionsToRecord(&$options, &$record) {
        ...
        if (is_string($options['sql'])) {
            ...
            if (isset($sql['query'])) {
                ...
                throw new XML_Query2XML_ConfigException(
                'The configuration option "query" is missing.', 'sql'
                );
            }
                ...
                if (is_array($sql['data'])) {
                    for ($i = 0; $i < count($sql['data']); $i++) {
                        $sql['data'][$i] = $this->_applyColumnStringToRecord(
                                $sql['data'][$i], $record, 'data', 'sql'
                        );
                    }
                } else {
                    //unit test: applySqlOptionsToRecordWrongDataTypeException()
                    throw new XML_Query2XML_ConfigException(
                    'The configuration option "data" is not an array.', 'sql'
                    );
                }
            }
        }
                ...
                for ($ii = 0; $ii < count($merge_selective); $ii++) {
                    for ($i = 0; $i < count($records); $i++) {
                        if (!array_key_exists($merge_selective[$ii], $record)) {
                            ...
                            . 'exist in the result set.', 'sql_options'
                            );
                        }
                        //parent is master!
                        $records[$i][$merge_selective[$ii]] = $record[$merge_selective[$ii]];
    ...
    private function _applyColumnStringToRecord($columnStr, &$record, $optionName, $parentOptionName = '') {
        ...
        if (strpos($columnStr, ':') === 0) {
            ...
            if (array_key_exists($columnStr, $record)) {
                ...
                . '"' . $optionName . '" does not exist in the result set.', $parentOptionName
                );
            }
        }
        return $ret;
    ...
    private function _debugStopQuery($sql) {
        ...
        if ($this->_profiling) {
            ...
            $this->_profile['queries'][$query]['runTimes'][$lastIndex]['stop'] = microtime(true);
        }
    }
    ...
    private function _stopDBProfiling() {
        if ($this->_profiling && isset($this->_profile['start'])) {
            ...
            $this->_profile['dbDuration'] = $this->_profile['dbStop'] - $this->_profile['start'];
        }
    }
    ...
    private static function _createDOMElement(DomDocument $dom, $name, $value = false) {
        try {
    ...
    private static function _addDOMGrandchildren(DomNode $base, $grandchildren, $childTagName) {
