...
if (isset($_SESSION[usuario])) {
    ...
    // cambiar formato y habilitar para que solo sea tramitado para usos personales, los cierres no son necesarios porque registro y estadistica no los solicita porque se alimenta la base de datos
        ...
        if ($fecha_actual >= $fecha_desabilita && $fecha_actual <= $fecha_habilitada /*&& $carnet <> 202099999*/) {
            $aviso = true;
            $mensaje = "Las solicitud en línea de cierres de pensum será habilitada a partir de enero del 2025 de nuevo.";
            ...
            if ($db->isError($carreras)) {
                ...
                if (count($carreras) == 0) {
                    ...
                    /*$consulta = "SELECT *
                    FROM constancias_cierre c
                    WHERE c.carnet = $carnet and (c.estado = 0 or (c.fecha_impresion is not null and c.fecha_impresion >= (curdate() - interval 1 month) ))
                    order by c.fecha_solicitud desc  limit 1";*/
                    ...
                    if ($db->isError($solicitudes_cola)) {
                        ...
                        if (count($solicitudes_cola) <> 0 && $carnet != 100024618) {
                           // print_r($solicitudes_cola);
                           $aviso = true;
                           // foreach($solicitudes_cola as $sc){
                                //if($sc[estado] == 0){
                               ...
                               /*
                                if($sc[estado] == 1){
                                    $mensaje = "La última solicitud que realizaste fue hace menos de 1 mes, si no has pasado a recoger la constancia puedes hacerlo en recepción de Arquitectura.";
                                }*/
                           // }
                            $url = "../menus/contenido.php";
                        }
