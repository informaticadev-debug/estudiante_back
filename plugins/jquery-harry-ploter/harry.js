...
//TOOLS LIB ===================================================================
                    ...
                    getRGB = function (color) {
                        ...
                        if (color && color.constructor == Array && color.length == 3)
                            return color;
                        if (r = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/.exec(color))
                            return [parseInt(r[1], 10), parseInt(r[2], 10), parseInt(r[3], 10)];
                        if (r = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)/.exec(color))
                            return [parseFloat(r[1]) * 2.55, parseFloat(r[2]) * 2.55, parseFloat(r[3]) * 2.55];
                        if (r = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(color))
                            return [parseInt(r[1], 16), parseInt(r[2], 16), parseInt(r[3], 16)];
                        if (r = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(color))
                    ...
                    calcColor = function (color, delta, alpha) {
                        ...
                        for (i = 0; i < 3; ++i)
                            rgb[i] = Math.max(Math.min(rgb[i] + delta[i], 255), 0);
                    ...
                    fontPixSize = function (f) {
                        ...
                        p = f.match(/[0-9\.]+em/i);
                        if (p)
                            return Math.floor(16.0 * parseFloat(p));
                        p = f.match(/\d+pt/i);
                        if (p)
                            return Math.floor(1.3333 * parseInt(p, 10));
                        return 10;
                    },
                    scaleUp = function (n) {
                    ...
                    textBox = function (gc, t, v, m, b) {
                        ...
                        b.lh = fontPixSize(gc.font) + m,
                                ...
                                b.w = 0;
                        for (n in b.lines)
                            if ((w = gc.measureText(b.lines[n]).width + m * 2) > b.w)
                                b.w = w;
                        return b;
                    },
//PRIVATE VARS ================================================================
                    ...
                    setup = function () {
                        ...
                        if (dlen) {
                            for (i = 0; i < dlen; i++) {
                                ...
                                if (d.maxlab)
                                    labels.xwidth = Math.max(labels.xwidth, gc.measureText(d.maxlab).width);
                            }
                            if (scaletop)
                                dmax = scaleUp(dmax);
                            if (flag.stack) {
                        ...
                        if (/none|false/.test(grid.y))
                            grid.y = [];
                        //plot zone
                        if (automargins)
                            margins = calcMargins(flag, labels, mo);
                        rx = margins[3];
                        ry = margins[0];
                    ...
                    plot = {
                        ...
                        chart: function () {
                            ...
                            if (dlen) {
                                ...
                                if (bw < 0)
                                    bw = 0;
                                for (y = fv?ry:rx, g = (fv?rh:rw) / (nbd || 1), nd = 0; nd <= nbd; nd++)
                                    gx.push(Math.round(y + nd * g));
                                drawXGrid(gx);
                                for (nds = 0; nds < dlen; nds++)
                                    overpts.push({x: [], y: [], v: [], nds: nds});
                                if (fv)
                    ...
                    over = {
                        ...
                        pie: function (x, y) {
                            ...
                            if (d <= overpie.r) {
                                ...
                                while (i < overpts.length && a > overpts[i].a)
                                    i++;
                                if (--i < 0)
                                    i = overpts.length - 1;
