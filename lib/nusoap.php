...
class wsdl extends nusoap_base {
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (!xml_parse($this->parser, $wsdl_string, true)) {
            // Display an error message.
            $errstr = sprintf(
                    'XML error parsing WSDL from %s on line %d: %s', $wsdl, xml_get_current_line_number($this->parser), xml_error_string(xml_get_error_code($this->parser))
            );
            $this->debug($errstr);
            $this->debug("XML payload:\n" . $wsdl_string);
            $this->setError($errstr);
            ...
            if ($pos_xml !== FALSE) {
                ...
                if (preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, $res)) {
                    ...
                    if (strtoupper($xml_encoding) != $encoding) {
                        ...
                        $this->debug($err);
                        if ($encoding != 'UTF-8' || strtoupper($xml_encoding) != 'UTF-8') {
                            $this->setError($err);
                            return;
                        }
                        // when HTTP says UTF-8 (the default) and XML says UTF-8 (the typical), assume the other endpoint is just sloppy and proceed
                    } else {
