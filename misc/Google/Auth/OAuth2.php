...
class Google_Auth_OAuth2 extends Google_Auth_Abstract {
    ...
    public function authenticate($code, $crossClient = false) {
        ...
        $request = new Google_Http_Request(
                self::OAUTH2_TOKEN_URI, 'POST', array(), $arguments
        );
        $request->disableGzip();
        ...
        if ($response->getResponseHttpCode() == 200) {
            ...
            sprintf(
                    "Error fetching OAuth2 access token, message: '%s'", $errorText
    ...
    private function refreshTokenRequest($params) {
        ...
        $http = new Google_Http_Request(
                self::OAUTH2_TOKEN_URI, 'POST', array(), $params
        );
        $http->disableGzip();
    ...
    public function revokeToken($token = null) {
        ...
        $request = new Google_Http_Request(
                self::OAUTH2_REVOKE_URI, 'POST', array(), "token=$token"
        );
        $request->disableGzip();
        $response = $this->client->getIo()->makeRequest($request);
    ...
    $jwt, $certs, $required_audience, $issuer = null, $max_expiry = null
    ) {
        if (!$max_expiry) {
        ...
        if ($now < $earliest) {
            throw new Google_Auth_Exception(
            sprintf(
                    "Token used too early, %s < %s: %s", $now, $earliest, $json_body
            )
            );
        }
        if ($now > $latest) {
            throw new Google_Auth_Exception(
            sprintf(
                    "Token used too late, %s > %s: %s", $now, $latest, $json_body
            )
            );
        }
        ...
        if ($issuer && $iss != $issuer) {
            throw new Google_Auth_Exception(
            sprintf(
                    "Invalid issuer, %s != %s: %s", $iss, $issuer, $json_body
            )
            );
        }
        ...
        if ($aud != $required_audience) {
            throw new Google_Auth_Exception(
            sprintf(
                    "Wrong recipient, %s != %s:", $aud, $required_audience, $json_body
            )
            );
        }
