...
function eliminarSolicitudesRealizadas($db, $carnet, $anio, $semestre, $evaluacion) {
    $consulta = "
                DELETE FROM asignacion_solicitud
                WHERE anio = $anio AND semestre = $semestre AND evaluacion = $evaluacion AND carnet = $carnet
            ";
    $solicitud_previa = & $db->query($consulta);
    if ($db->isError($solicitud_previa)) {
        return false;
    } else {
        return $solicitud_previa;
...
function obtenerCursosASolicitar($db, $carnet, $carrera, $anio, $semestre, $extension, $evaluacion) {
    $consulta = "
                SELECT ao.pensum, ao.codigo, CONCAT('Ciclo: ', c.ciclo, ' - ', IF(c.caracter = 'F' , 'Fundamental', 'Electiva'), ' - ', c.nombre, ' (', ao.observacion, ' horas)') as nombre
...
if (isset($_SESSION[usuario])) {
    ...
    if (DB::isError($db)) {
        ...
        if (/*$inscripcion[0]["carrera"] != 1 ||*/ $extension == 12) {
        ...
        if ($db->isError($inscripcion)) {
            ...
            if (!empty($_POST["action"]) && $_POST["action"] == "del") {
                eliminarSolicitudesRealizadas($db, $carnet, $anio, $semestre, $evaluacion);
            }
                ...
                if ($db->isError($ciclo)) {
                    ...
                    if ($inscripcion == 0) {
                        ...
                        if ($ciclo == 0) {
                            ...
                            if ($ciclo[fecha_inicio_asignacion] > $ciclo[fecha_actual] && $permitir_ingreso <> $carnet && false) {
                                $error = true;
        ...
        if (!$error && !$aviso) {
            ...
            $template->loadTemplateFile('asignacion_interciclos_solicitud_pre.html');
            ...
            if (!empty($a) && count($solicitud_anterior) < 1 && count($asignaciones) < 2) {
                foreach ($a as $curso_disponible) {
                    $template->setVariable(array(
                        ...
                        'value_curso2' => $anio . "_" . $semestre . "_" . $evaluacion . "_" . $curso_disponible['pensum'] . "_" . $curso_disponible['codigo'],
                        'descripcion_curso' => $curso_disponible['codigo'] . " - " . $curso_disponible['nombre'],
                        'descripcion_curso2' => $curso_disponible['codigo'] . " - " . $curso_disponible['nombre'],
                    ));
                    $template->parse('cursos_disponibles');
                    $template->parse('cursos_disponibles2');
                }
            } else {
                if (count($asignaciones) > 2) {
                    ...
                    $extra = "";
                    if (count($solicitud_anterior) > 1) {
                        $extra = " y {$solicitud_anterior[1]["codigo"]} - {$solicitud_anterior[1]["nombre"]}";
                    }
                    $template->setVariable(array(
                        'sin_asignaciones_disponibles' => "<div class='alert alert-success'>Ya cuenta con una solicitud, ha solicitado: <b>" . $solicitud_anterior[0]['codigo'] . " - " . trim($solicitud_anterior[0]['nombre']) . "$extra</b>.</div>
                            <div class='row'>
                                        <form method='POST'>
                                            <input type='hidden' name='action' value='del' />
                                            <input type='submit' class='btn btn-danger' style='margin-left: 15px;' value='Eliminar respuesta actual y volver a responder' />
                                        </form>
                                    </div>
                            ",
                        'ocultar_seleccion_cursos' => 'hidden'
