...
if (isset($_SESSION[usuario])) {
    ...
    if (DB::isError($db)) {
        ...
        GROUP BY i.carrera";
        //if ($carnet == 100024618) {var_dump($consulta); die;}
        ...
        if (!$error && !$aviso) {
            ...
            if (!empty($a)) {
                ...
                foreach ($a as $cu) {
                    ...
                    //si no tiene secciones en Activas, ignorar el curso, paso el 2020-1, había solo una sección eliminada y salía con cupo sin secciones.
                    if (empty($cu["secciones"]))
                        continue;
                    ...
                    if (!empty($seccion)) {
                        ...
                        foreach (explode(",", $cu["secciones"]) as $seccionCE) {
                            $template->setVariable(
                                array(
                                    'pensum_sec' => $cu["pensum"],
                                    'codigo_sec' => $cu["codigo"],
                                    'seccion_sec' => 'spd_' . $seccionCE,
                                    'seccion_sec_desc' => 'Solicitar Cupo Extra - ' . $seccionCE,
                                )
                            );
                            $template->parse('secciones_disponibles');
                        }
